...
if "current_course_code" not in st.session_state:
    st.session_state.current_course_code = None  # 鎖定課程編號

if "forced_language" not in st.session_state:
    st.session_state.forced_language = None  # 預設未被用戶強制鎖定語言
...
if user_query := st.chat_input("您可以問我：IT114126有咩讀？學費幾多？入學要求係咩？"):
    ...
    # ==================== 6. 調用本地 Ollama 運作模型（帶歷史記憶鏈與讀取動畫） ====================
    with st.chat_message("assistant"):
        response_placeholder = st.empty()
        full_response = ""
        
        # 1. 建立符合 Ollama 規範的歷史記憶矩陣
        ...
        try:
            ...
            # 【亮點功能】：加入 spinner 動畫，直到模型計算完畢吐出「第一個字」
            with st.spinner("AI 深度思考與檢索資料中... "):
                try:
                    # 強制等待並抓取第一個字 (Chunk) 來打破動畫
            ...
            st.error(f"🔌 與本地 Ollama 引擎連線失敗，請確認是否已啟動 Ollama。詳細錯誤：{str(e)}")
